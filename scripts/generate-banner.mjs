...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="title desc">
  ...
  <style>
    ...
    .accent { fill: #56f3e8; }
    .source {
      animation: sourceBreathe 10s ease-in-out infinite;
    }
    .mark {
    ...
    .ambient-right {
      animation: ambientRight 16s ease-in-out infinite;
      transform-box: fill-box;
      transform-origin: center;
    }
    .ambient-bottom {
      animation: ambientBottom 20s ease-in-out infinite;
      transform-box: fill-box;
      transform-origin: center;
    }
    .scan {
      animation: scanPass 18s ease-in-out infinite;
      transform-box: fill-box;
      transform-origin: center;
    }
    ...
    @keyframes ambientRight {
      ...
      50% {
        opacity: 0.32;
        transform: translate3d(-34px, 8px, 0) scale(1.035);
      }
    }
    @keyframes ambientBottom {
      0%, 100% {
        opacity: 0.16;
        transform: translate3d(0, 0, 0) scale(1);
      }
      50% {
        opacity: 0.26;
        transform: translate3d(42px, -8px, 0) scale(1.04);
      }
    }
    @keyframes scanPass {
      0%, 34%, 100% {
  ...
  <defs>
    ...
    <linearGradient id="scanGradient" x1="0" y1="0" x2="420" y2="0" gradientUnits="userSpaceOnUse">
      <stop offset="0" stop-color="#ffffff" stop-opacity="0"/>
      <stop offset="0.46" stop-color="#ffffff" stop-opacity="0.48"/>
      <stop offset="1" stop-color="#56f3e8" stop-opacity="0"/>
    </linearGradient>
    <filter id="softBlur" x="-20%" y="-20%" width="140%" height="140%" color-interpolation-filters="sRGB">
      <feGaussianBlur stdDeviation="34"/>
    </filter>
    ...
    <filter id="markGlow" x="-45%" y="-45%" width="190%" height="190%" color-interpolation-filters="sRGB">
      <feDropShadow dx="0" dy="0" stdDeviation="13" flood-color="#ff3e3b" flood-opacity="0.28"/>
    </filter>
  </defs>
  ...
  <rect width="${width}" height="${height}" fill="url(#rightGlow)"/>
  <rect width="${width}" height="${height}" fill="url(#bottomGlow)"/>
  <g class="ambient-right">
    <ellipse cx="1268" cy="172" rx="292" ry="108" fill="#7e878c" filter="url(#softBlur)"/>
  </g>
  <g class="ambient-bottom">
    <ellipse cx="603" cy="391" rx="390" ry="72" fill="#a4a9a6" filter="url(#softBlur)"/>
  </g>
  <rect class="scan" x="-420" y="0" width="420" height="${height}" fill="url(#scanGradient)" transform="skewX(-16)"/>
